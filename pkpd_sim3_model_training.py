...
print('loading data')
n_units, n_treated, n_units_total, step, train_step, control_sample, noise, n_basis, n_cluster = io_utils.load_config(data_path, 'train')
x_full, t_full, mask_full, batch_ind_full, y_full, y_control, y_mask_full, m, sd, treatment_effect = io_utils.load_tensor(data_path, 'train')
x_full_val, t_full_val, mask_full_val, batch_ind_full_val, y_full_val, y_control_val, y_mask_full_val, _, _, _ = io_utils.load_tensor(data_path, 'val')
...
for i in range(itr):
    ...
    if pretrain_Y:
        if not linear_decoder:
            dec_Y = SyncTwin.RegularDecoder(hidden_dim=enc.hidden_dim, output_dim=y_full.shape[-1], max_seq_len=step - train_step)
        else:
    ...
    nsc = SyncTwin.SyncTwin(n_units, n_treated, reg_B=0., lam_express=1., lam_recon=lam_recon, lam_prognostic=lam_prognostic, tau=tau, encoder=enc, decoder=dec, decoder_Y=dec_Y)

    print('Pretrain')
    if not pretrain_Y:
        train_utils.pre_train_reconstruction_loss(nsc, x_full, t_full, mask_full, x_full_val, t_full_val, mask_full_val,
                                                  niters=itr_pretrain, model_path=model_path, batch_size=batch_size)
    else:
        train_utils.pre_train_reconstruction_prognostic_loss(nsc, x_full, t_full, mask_full, y_full, y_mask_full,
                                                             x_full_val, t_full_val, mask_full_val, y_full_val, y_mask_full_val,
                                                  niters=itr_pretrain, model_path=model_path, batch_size=batch_size)

    if not reduced_fine_tune:
        return_code = train_utils.train_all_losses(nsc, x_full, t_full, mask_full, batch_ind_full,
                                                   y_full, y_control, y_mask_full,
                                                   niters=itr_pretrain, model_path=model_path, batch_size=None)

    print('Eval')
    print('Matching on validation set')
    train_utils.load_pre_train_and_init(nsc, x_full_val, t_full_val, mask_full_val, batch_ind_full_val, model_path=model_path,
                                        init_decoder_Y=pretrain_Y)
    return_code = train_utils.train_B_self_expressive(nsc, x_full_val, t_full_val, mask_full_val, batch_ind_full_val,
                                    niters=itr_fine_tune, model_path=model_path)
...
if pretrain_Y:
    if not linear_decoder:
        dec_Y = SyncTwin.RegularDecoder(hidden_dim=enc.hidden_dim, output_dim=y_full.shape[-1],
                                        max_seq_len=step - train_step)
    else:
        dec_Y = SyncTwin.LinearDecoder(hidden_dim=enc.hidden_dim, output_dim=y_full.shape[-1],
                                       max_seq_len=step - train_step)
...
for i in range(itr):
    ...
    nsc = SyncTwin.SyncTwin(n_units, n_treated, reg_B=0., lam_express=1., lam_recon=lam_recon, lam_prognostic=lam_prognostic, tau=tau, encoder=enc, decoder=dec, decoder_Y=dec_Y)
    train_utils.load_pre_train_and_init(nsc, x_full, t_full, mask_full, batch_ind_full, model_path=model_path,
                                            init_decoder_Y=pretrain_Y)
    return_code = train_utils.train_B_self_expressive(nsc, x_full, t_full, mask_full, batch_ind_full,
                                            niters=itr_fine_tune, model_path=model_path_save)
    if return_code != 0:
        control_error_list.append(1E9)
        continue
