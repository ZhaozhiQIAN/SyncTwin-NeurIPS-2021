  ...
  def fit_transform(self, dataset):    
    if len(self.temporal_feature_names) > 0:
      if dataset.temporal_data is not None:
        dataset.temporal_data, temporal_norm_parameters = normalizer(dataset.temporal_data,
                                                                     self.temporal_feature_names,
                                                                     normalizer_type = 'minmax')
    if len(self.static_feature_names) > 0:
      if dataset.static_data is not None:
        dataset.static_data, static_norm_parameters = normalizer(dataset.static_data,
                                                                 self.static_feature_names,
                                                                 normalizer_type = 'minmax')
    norm_parameters = {'normalizer': 'minmaxscaler',
    ...
    if len(self.static_feature_names) > 0:
      if dataset.static_data is not None:
        dataset.static_data, static_norm_parameters = normalizer(dataset.static_data, 
                                                                 self.static_feature_names, 
                                                                 normalizer_type = 'standard')
    norm_parameters = {'normalizer': 'standardscaler',
                       'temporal': temporal_norm_parameters,
                       'static': static_norm_parameters,}
...
class ReScaler(BaseEstimator, DataPreprocessorMixin):
  ...
  def fit_transform(self, dataset):    
    if dataset.temporal_data is not None and self.temporal_norm_parameters is not None:
        dataset.temporal_data = renormalizer(dataset.temporal_data, 
                                             self.temporal_norm_parameters)  
    if dataset.static_data is not None and self.static_norm_parameters is not None:
      if dataset.static_data is not None:
        dataset.static_data = renormalizer(dataset.static_data, 
                                           self.static_norm_parameters)
