...
def normalization_module (x, s):
  ...
  x[temporal_col_names], temporal_norm_parameters = \
  normalization(x[temporal_col_names])
  
  s[static_col_names], static_norm_parameters = \
  normalization(s[static_col_names])
  
  norm_parameters = {'temporal_max_val': temporal_norm_parameters['max_val'],
...
def data_preprocess (train_x, train_s, test_x, test_s,
                     preprocess_parameters):
  '''Preprocess the data.
  (1) One-hot encoding
  (2) Normalization
  ...
  train_x, train_s = one_hot_encoding_module (train_x, train_s,  
                                              one_hot_encoding_features)
  test_x, test_s = one_hot_encoding_module (test_x, test_s,  
                                            one_hot_encoding_features)
  ...
  if normalization:
    train_x, train_s, normalization_parameters = \
    normalization_module (train_x, train_s)
    test_x, test_s, _ = \
    normalization_module (test_x, test_s)
  ...
  for dict_name in train_data.keys():
    if train_data[dict_name].shape[0] == set_length:      
      train_data[dict_name] = temporal_data_formatting(train_data[dict_name], 
                                                       max_seq_len)  
      test_data[dict_name] = temporal_data_formatting(test_data[dict_name], 
                                                      max_seq_len)  
    else:
