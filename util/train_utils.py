...
def pre_train_reconstruction_loss(nsc, x_full, t_full, mask_full, x_full_val=None, t_full_val=None, mask_full_val=None,
                                  niters=5000, model_path='models/sync/{}.pth', batch_size=None):
    ...
    optimizer = optim.Adam(list(dec.parameters()) +
                           list(enc.parameters()))
...
def pre_train_reconstruction_prognostic_loss(nsc, x_full, t_full, mask_full, y_full, y_mask_full,
                                    x_full_val=None, t_full_val=None, mask_full_val=None, y_full_val=None, y_mask_full_val=None,
                                  niters=5000, model_path='models/sync/{}.pth', batch_size=None):
    if x_full_val is None:
        x_full_val = x_full
        t_full_val = t_full
    ...
    for itr in range(1, niters + 1):
        ...
        if batch_size is not None:
            x, t, mask, y, y_mask = batching.get_batch_standard(batch_size, x_full, t_full, mask_full, y_full, y_mask_full)
        else:
            ...
            with torch.no_grad():
                if x_full_val.shape[1] < 5000:
                    ...
                    for fold in range(n_fold):
                        x_full_vb, t_full_vb, mask_full_vb, y_full_vb, y_mask_full_vb = batching.get_folds(fold, n_fold,
                                                                                                           x_full_val,
                                                                                                           t_full_val,
                                                                                                           mask_full_val,
                                                                                                           y_full_val,
...
def train_cfr(cfr, x_full, t_full, mask_full, y_full, y_mask_full,
              x_full_val=None, t_full_val=None, mask_full_val=None, y_full_val=None, y_mask_full_val=None,
              niters=5000, model_path='', batch_size=None):
    if x_full_val is None:
        x_full_val = x_full
        t_full_val = t_full
        mask_full_val = mask_full
    ...
    optimizer = optim.Adam(list(enc.parameters()) +
                           list(dec_Y.parameters()))
...
def load_pre_train_and_init(nsc, x_full, t_full, mask_full, batch_ind_full, model_path='models/sync/{}.pth', init_decoder_Y=False):
    enc = nsc.encoder
    dec = nsc.decoder
...
def train_B_self_expressive(nsc, x_full, t_full, mask_full, batch_ind_full,
                            niters=20000, model_path='models/sync/{}.pth', batch_size=None, lr=1e-3, test_freq=1000):
...
def train_all_losses(nsc, x_full, t_full, mask_full, batch_ind_full, y_full, y_control, y_mask_full,
                     niters=10000, model_path='models/sync/{}.pth', batch_size=None):
    ...
    for itr in range(1, niters + 1):
        ...
        loss, C = nsc(x_full, t_full, mask_full, batch_ind_full, y_batch=y_full, y_control=y_control, y_mask=y_mask_full)
        loss.backward()
        optimizer.step()
