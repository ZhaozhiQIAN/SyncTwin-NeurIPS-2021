...
def get_summary(model_id):
    ...
    x_full, t_full, mask_full, batch_ind_full, y_full, y_control, y_mask_full, m, sd, treatment_effect = io_utils.load_tensor(data_path)
    ...
    if model_id.find('none') > 0:
        nsc = SyncTwin.SyncTwin(n_units, n_treated, reg_B=0., lam_express=1., lam_recon=1., lam_prognostic=0., tau=0.3, encoder=enc, decoder=dec)
    else:
        nsc = SyncTwin.SyncTwin(n_units, n_treated, reg_B=0., lam_express=1., lam_recon=1., lam_prognostic=0., tau=0.3, encoder=enc, decoder=dec, decoder_Y=dec_Y)
