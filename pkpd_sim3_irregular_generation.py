...
for fold in ['train', 'val', 'test']:
    ...
    missing = (missing < missing_pct) * 1.

    mask_full_bool = (mask_full + (1 - missing) == 2)
    mask_full = mask_full_bool * 1.
    ...
    mat_imputed = mat_imputed.reshape(x_full_zero.shape[1], x_full_zero.shape[0], x_full_zero.shape[2]).transpose((1, 0, 2))
