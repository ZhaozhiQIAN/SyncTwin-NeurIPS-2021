...
for i in range(itr):
    ...
    nsc = SyncTwin.SyncTwin(n_units_train, n_treated_train, reg_B=0., lam_express=1., lam_recon=1.,
                            lam_prognostic=lam_prognostic, tau=tau, encoder=enc, decoder=dec, decoder_Y=dec_Y,
                            reduce_gpu_memory=True)

    final_loss = train_utils.pre_train_reconstruction_prognostic_loss(nsc, x_full, t_full, mask_full, y_full, y_mask_full,
                                                         x_full_val, t_full_val, mask_full_val, y_full_val,
                                                         y_mask_full_val,
